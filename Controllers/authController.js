...
module.exports.login = (request, response, next) => {
  let token1;
  //connection db findOne() // here checkthrough database if that person is admin
  if (request.body.email == "ahmed@gmail.com" && request.body.password == 123) {
    token1 = jwt.sign(
      {
        _id: 1, //eldata elle ha7otaha gowa el token w hattb3t llserver
        email: request.body.email,
        role: "admin",
      },
      "secretKey"
    ); // THIS SHOULD BE GENERATED BY CRYPTO
    response.status(200).json({ msg: "admin login", token1 });
  } else if (request.body.role == "speaker") {
    Speaker.findOne({ Email: request.body.Email })
      .then((data) => {
    ...
    Student.findOne({ email: request.body.email })
      .then((data) => {
        SpeakerStudentLogin(data, "student", request, response);
      })
      .catch((error) => next(error));
  }
};
...
function SpeakerStudentLogin(data, role, request, response) {
  ...
  response
    .status(200)
    .json({ message: role + " logged in successfully", token1 });
}
